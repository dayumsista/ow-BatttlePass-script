...
def perform_actions():
    ...
    while not stop_flag.is_set():
        ...
        # 模拟游戏输入
        time.sleep(11)
        ctypes.windll.user32.keybd_event(0x1B, 0, 0, 0)  # ESC 按下
        ctypes.windll.user32.keybd_event(0x1B, 0, 2, 0)  # ESC 释放

        time.sleep(0.5)
        user32.SetCursorPos(1260, 881)
        time.sleep(0.5)
        ctypes.windll.user32.mouse_event(2, 0, 0, 0, 0)  # 鼠标左键按下
        ctypes.windll.user32.mouse_event(4, 0, 0, 0, 0)  # 鼠标左键释放

        time.sleep(0.2)
        ctypes.windll.user32.keybd_event(0x20, 0, 0, 0)  # SPACE 按下
        ctypes.windll.user32.keybd_event(0x20, 0, 2, 0)  # SPACE 释放
